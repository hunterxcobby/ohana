...
foreach ($garment_data as $row): ?>
		...
		<div class="">
			...
			<div class="well well-sm" style="background:#F5FFFA;margin-top:-9px;">
				...
				<h5 style="margin-left:10px;margin-top:20px;"> <?php echo lang('Unit'); ?>  : <input class="form-control"  id="spinner3" name="quantity"  value="1" autofocus/>

				<?php 
					  if($ServiceUnit=="Kg" || $ServiceUnit=="Mtr") { echo $ServiceUnit; $step="0.1";}
					  else {  echo $ServiceUnit; $step="1"; } 
				?> 
				</h5>
				...
				<div class="form-group" style="margin-top:15px;">
						...
						<table  class="table table-hover small-text" id="tb">
							...
							<select name="gar_desc_kg[]" class="form-control">
							   <option value="" selected> <?php echo lang('Select Garment'); ?>  </option>
			...
			jQuery(function($) {
					...
					$('#div_kg').hide(); 	
					 $('#unit_type').change(function(){
						...
						if($('#unit_type').val() == 'unit_kg') {
							$('#div_kg').show();
							$('#div_qty').hide(); 							
						} 
						
						if($('#unit_type').val() == 'unit_qty') {
							$('#div_qty').show(); 
							$('#div_kg').hide();
						} 
